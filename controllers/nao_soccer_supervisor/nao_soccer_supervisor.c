...
static Robot *robot_new(WbNodeRef node) {
  ...
  // get reference to waist band color so that we can change it
  WbFieldRef field = wb_supervisor_node_get_field(node, "children");
  node  = wb_supervisor_field_get_mf_node(field, 0);
  field = wb_supervisor_node_get_field(node, "children");
  node  = wb_supervisor_field_get_mf_node(field, 0);
  field = wb_supervisor_node_get_field(node, "appearance");
  node  = wb_supervisor_field_get_sf_node(field);
  field = wb_supervisor_node_get_field(node, "material");
  node  = wb_supervisor_field_get_sf_node(field);
  robot->waist_band_color = wb_supervisor_node_get_field(node, "diffuseColor");
...
static void place_to_kickoff() {
  ...
  move_robot_2d(get_blue_robot_index(0), -GOALIE_X, 0, 1.5708);
  move_robot_2d(get_red_robot_index(0), GOALIE_X, 0, -1.5708);

  // move other robots
  if (control_data.kickOffTeam == TEAM_RED) {
    move_robot_2d(get_red_robot_index(1), KICK_OFF_X, 0, -1.5708);
    move_robot_2d(get_red_robot_index(2), PENALTY_BALL_X_POS, PENALTY_AREA_Z_DIM / 2, -1.5708);
    move_robot_2d(get_red_robot_index(3), DEFENDER_X,        -PENALTY_AREA_Z_DIM / 2, -1.5708);
    
    move_robot_2d(get_blue_robot_index(1), -DEFENDER_X, -GOAL_WIDTH / 4.0, 1.5708);
    move_robot_2d(get_blue_robot_index(2), -DEFENDER_X, -KICK_OFF_Z, 1.5708);
    move_robot_2d(get_blue_robot_index(3), -DEFENDER_X,  KICK_OFF_Z, 1.5708);
  }
  else {
    move_robot_2d(get_red_robot_index(1), DEFENDER_X,  KICK_OFF_Z, -1.5708);
    move_robot_2d(get_red_robot_index(2), DEFENDER_X, -KICK_OFF_Z, -1.5708);
    move_robot_2d(get_red_robot_index(3), DEFENDER_X,  GOAL_WIDTH / 4.0, -1.5708);
    
    move_robot_2d(get_blue_robot_index(1), -DEFENDER_X,          PENALTY_AREA_Z_DIM / 2, 1.5708);
    move_robot_2d(get_blue_robot_index(2), -KICK_OFF_X, 0, 1.5708);
    move_robot_2d(get_blue_robot_index(3), -PENALTY_BALL_X_POS, -PENALTY_AREA_Z_DIM / 2, 1.5708);
  }
